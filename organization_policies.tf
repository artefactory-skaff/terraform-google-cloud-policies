locals {
  common_organization_policies = [
    ...
    { constraint = "compute.sharedReservationsOwnerProjects", rules = [{ type = "Allow", values = ["projects/${var.admin_project_id}"] }] },
    { constraint = "compute.storageResourceUseRestrictions", rules = [{ type = "Allow", values = ["under:${var.parent_id}"] }] },
    { constraint = "compute.trustedImageProjects", rules = [{ type = "Allow", values = ["projects/${var.admin_project_id}"] }] },
    ...
    { constraint = "resourcemanager.allowedExportDestinations", rules = length(var.export_candidate_parent_ids) == 0 ? [{ deny_all = "TRUE", type = null, values = null }] : [{ deny_all = null, type = "Allow", values = concat([for parent_id in var.export_candidate_parent_ids : "under:${parent_id}"], []) }] },
